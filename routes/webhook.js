...
router.post('/stripe', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  try {
    if (event.type === 'checkout.session.completed') {
      ...
      if (!existingLicense) {
        ...
        await AuditLog.create({
          ...
          email,
          status: 'success',
          details: `Stripe webhook processed: ${event.type}`
        });
        ...
        try {
          ...
          await AuditLog.create({
            ...
            email,
            status: 'success',
            details: `License email sent for ${licenseKey}`
          });
        } catch (emailError) {
    ...
    if (event.type === 'invoice.payment_succeeded' || event.type === 'invoice.paid') {
      ...
      let existingLicense = await License.findOne({ email, status: 'active' });
      ...
      if (!existingLicense) {
        ...
        await AuditLog.create({
          ...
          email,
          status: 'success',
          details: `Stripe invoice webhook processed: ${event.type}`
        });
